...
from ldm.models.diffusion.DC_sync_dreamer import DCSyncDiffusion, DCSyncDDIMSampler
...
def prepare_inputs_control(input_path_list, input_azimuth_list, camera_distance, elevation, crop_size=-1, image_size=256):
    # data = {}
    ...
    for target_pose in poses:
        ...
        Ts.append(torch.stack(T_list, -1))
        lam_org.append(torch.stack(lam_list, -1))
    ...
    Ts = torch.stack(Ts, -1)  # (4, s, 16)
    lam_org = torch.stack(lam_org, -1)  # (s, 16)
    camera_para = torch.stack(camera, -1)  # (25, 16)
...
def main():
    ...
    parser.add_argument('--cfg',type=str, default='configs/DC_syncdreamer.yaml')
    parser.add_argument('--ckpt',type=str, default='ckpt/DC_syncdreamer.ckpt')
    parser.add_argument('--output', type=str, default='output_imgs/dc_syncdreamer/alarm')
    ...
    image_size = 256
    data = prepare_inputs_control(input_path_list, input_azimuth_list, camera_distance, elevation, crop_size, image_size)
    ...
    for bi in range(B):
        imsave(join(output_path, f'{bi}.png'), np.concatenate([x_sample[bi,ni] for ni in range(N)], 1))
