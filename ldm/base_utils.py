...
def concat_images(img0, img1, vert=False):
    if not vert:
        # If concatenating horizontally, match heights
        h0, h1 = img0.shape[0], img1.shape[0]
        padding_h0 = h1 - h0 if h0 < h1 else 0
        padding_h1 = h0 - h1 if h1 < h0 else 0
        img0 = np.pad(img0, ((0, padding_h0), (0, 0), (0, 0)), mode='constant', constant_values=0)
        img1 = np.pad(img1, ((0, padding_h1), (0, 0), (0, 0)), mode='constant', constant_values=0)
        img = np.concatenate([img0, img1], axis=1)
    else:
        # If concatenating vertically, match widths
        w0, w1 = img0.shape[1], img1.shape[1]
        padding_w0 = w1 - w0 if w0 < w1 else 0
        padding_w1 = w0 - w1 if w1 < w0 else 0
        img0 = np.pad(img0, ((0, 0), (0, padding_w0), (0, 0)), mode='constant', constant_values=0)
        img1 = np.pad(img1, ((0, 0), (0, padding_w1), (0, 0)), mode='constant', constant_values=0)
        img = np.concatenate([img0, img1], axis=0)
    return img
